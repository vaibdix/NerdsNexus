...
const Cart = () => {
  ...
  return (
    <motion.div
      {...slideIn}
      className="fixed z-50 top-0 right-0 w-300 md:w-508 bg-[#191414] backdrop-blur-md shadow-md h-screen"
    >
      <div className="w-full flex items-center justify-between py-4 px-2">
        ...
        >
          <BiChevronsRight className="text-[50px] text-gray-300" />
        </motion.i>
        <p className="text-xl text-white font-semibold">Your Cart</p>
        <motion.i {...buttonClick} className="cursor-pointer">
          <FcClearFilters className="text-[30px] text-white" />
        </motion.i>
      </div>

      <div className="flex-1 flex flex-col items-start justify-start rounded-md bg-[#191414] h-full py-4 gap-4 relative">
        {cart && cart?.length > 0 ? (
          <>
            ...
            <div className="bg-zinc-800 rounded-md w-full flex flex-col items-center justify-center gap-5 pt-5 pb-5">
              <div className="w-full flex items-center justify-evenly">
                <p className="text-2xl text-zinc-500 ">
                  {/* Total */}
                  </p>
                <p className="text-2xl text-orange-500 flex items-center justify-center">
              ...
              <motion.button
                {...buttonClick}
                className="bg-orange-400 w-[70%] p-4 py-2 text-xl text-headingColor font-semibold hover:bg-orange-500 drop-shadow-md rounded-md"
...
export const CartItemCard = ({ index, data }) => {
  ...
  return (
    ...
    >
      <img
        src={data?.imageURL}
        className=" w-24 min-w-[94px] h-20 object-contain"
        alt=""
      />
